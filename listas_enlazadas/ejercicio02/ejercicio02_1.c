...
//Nueva función
int montoInvertido(nodoUsuario *listaUsuarios);
    ...
    do{
        ...
        if(opcion == 1){
            ...
            if(opcion == 2){
                ...
                if(opcion == 3){
                    printf("\nTOTAL INVERTIDO: %d\n", montoInvertido(usuariosConDeuda) + montoInvertido(usuariosSinDeuda));
                }
            }
        }
    }while(opcion != 4);
...
int montoInvertido(nodoUsuario *listaUsuarios){
    int montoTotalInv = 0;
    while(listaUsuarios){
        montoTotalInv += listaUsuarios->datos.montoOtorgado;
        listaUsuarios = listaUsuarios->siguiente;
    }

    return montoTotalInv;
}
